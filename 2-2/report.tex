...
\begin{itemize}
  \item 对平滑尺度做对比：$\sigma\in\{0.8,1.0,1.6\}$，观察模糊、梯度、NMS 与边缘图；
  \item 在基准 $\sigma=0.8$ 上，遍历三组双阈值（高/低比例）：$(0.25,0.10)$、$(0.35,0.15)$、$(0.15,0.07)$；
  \item 输出包含：模糊图、梯度幅值、方向、NMS 与对应边缘图，文件名带有参数后缀便于直接比对。
\end{itemize}
参数影响：
\begin{itemize}
...
平滑尺度对比见图~\ref{fig:task2-sigma}，双阈值对比见图~\ref{fig:task2-thresholds}。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.3\textwidth]{outputs/task2/blurred_sigma0.80.png}
  \includegraphics[width=0.3\textwidth]{outputs/task2/gradient_sigma0.80.png}
  \includegraphics[width=0.3\textwidth]{outputs/task2/edges_sigma0.80.png}
  \\
  \includegraphics[width=0.3\textwidth]{outputs/task2/blurred_sigma1.00.png}
  \includegraphics[width=0.3\textwidth]{outputs/task2/gradient_sigma1.00.png}
  \includegraphics[width=0.3\textwidth]{outputs/task2/edges_sigma1.00.png}
  \\
  \includegraphics[width=0.3\textwidth]{outputs/task2/blurred_sigma1.60.png}
  \includegraphics[width=0.3\textwidth]{outputs/task2/gradient_sigma1.60.png}
  \includegraphics[width=0.3\textwidth]{outputs/task2/edges_sigma1.60.png}
  \caption{不同 $\sigma$ 下的模糊、梯度幅值与边缘（均使用基准双阈值 0.25/0.10）}
  \label{fig:task2-sigma}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.32\textwidth]{outputs/task2/edges_thresh_high0.25_low0.10.png}
  \includegraphics[width=0.32\textwidth]{outputs/task2/edges_thresh_high0.35_low0.15.png}
  \includegraphics[width=0.32\textwidth]{outputs/task2/edges_thresh_high0.15_low0.07.png}
  \caption{基准 $\sigma=0.8$ 下不同双阈值的边缘结果}
  \label{fig:task2-thresholds}
\end{figure}

\subsection{任务三：Harris 角点检测}
\begin{itemize}
  \item 使用高斯导数求梯度，窗口平滑方差做对比：$\sigma_w\in\{0.8,1.5,2.5\}$，$\alpha=0.05$；
  \item 以响应最大值的 1\% 作为阈值并做 3$\times$3 NMS；
  \item 生成原图角点可视化、缩放 0.7 倍以及旋转 $30^\circ$ 后的角点可视化，用于观察尺度与旋转等变性；
  \item 窗口越大，响应更平滑、角点更稀疏，定位偏移也更明显；小窗口则角点密集但容易受噪声影响。
\end{itemize}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.32\textwidth]{outputs/task3/response_window0.80.png}
  \includegraphics[width=0.32\textwidth]{outputs/task3/response_window1.50.png}
  \includegraphics[width=0.32\textwidth]{outputs/task3/response_window2.50.png}
  \\
  \includegraphics[width=0.32\textwidth]{outputs/task3/corners_window0.80.png}
  \includegraphics[width=0.32\textwidth]{outputs/task3/corners_window1.50.png}
  \includegraphics[width=0.32\textwidth]{outputs/task3/corners_window2.50.png}
  \caption{不同窗口方差下的响应与角点分布对比}
  \label{fig:task3-windows}
\end{figure}
